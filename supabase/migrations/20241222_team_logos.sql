...
-- Generate badge-style logo URLs using DiceBear API based on team names
-- This creates football club crest-style logos for each team
UPDATE teams 
SET logo_url = CASE 
  ...
  -- City/Location teams get classic badge style
  ...
  -- Field/Nature locations get geometric badge patterns
  ...
  ELSE
    'https://api.dicebear.com/7.x/identicon/svg?seed=' || REPLACE(LOWER(name), ' ', '') || '&backgroundColor=' || REPLACE(primary_color, '#', '') || '&foregroundColor=' || REPLACE(secondary_color, '#', '')
END
WHERE logo_url IS NULL OR logo_url LIKE '%bottts%' OR logo_url LIKE '%fun-emoji%' OR logo_url LIKE '%thumbs%';